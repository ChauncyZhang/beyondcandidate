...
class IsolatedOcrRenderer:
    ...
    async def render_pdf(
        ...
        *,
        ...
        return await self._render(stream, source_type="pdf", limits=limits)
    ...
    async def render_image(
        ...
        *,
        ...
        return await self._render(stream, source_type="image", limits=limits)
    ...
    async def _render(
        ...
        *,
        ...
        if source_type == "pdf" and not source.startswith(b"%PDF-"):
            raise OcrRenderingError("file_magic_mismatch")
        if source_type not in {"pdf", "image"}:
            raise OcrRenderingError("ocr_source_type_invalid")

        request = json.dumps(
            {"limits": asdict(limits), "source_type": source_type},
            separators=(",", ":"),
        ).encode("utf-8") + b"\n" + source
        environment = {
            key: os.environ[key]
        ...
        try:
            ...
            process.kill()
            await process.wait()
            raise OcrRenderingError(f"{source_type}_render_timeout") from None
        ...
        if process.returncode != 0 or len(stdout) > limits.max_total_bytes + 65_536:
            raise OcrRenderingError(f"{source_type}_render_failed")
        ...
        try:
            ...
            raise OcrRenderingError(f"{source_type}_render_failed") from None
        if not isinstance(header, dict):
            raise OcrRenderingError(f"{source_type}_render_failed")
        if not header.get("ok"):
            raise OcrRenderingError(header.get("safe_code", f"{source_type}_render_failed"))
        ...
        for expected_number, item in enumerate(metadata, start=1):
            ...
            if media_type == "image/png" and not image.startswith(b"\x89PNG\r\n\x1a\n"):
                raise OcrRenderingError(f"{source_type}_render_failed")
            if media_type == "image/jpeg" and not image.startswith(b"\xff\xd8\xff"):
                raise OcrRenderingError(f"{source_type}_render_failed")
            pages.append(RenderedOcrPage(number, image, width, height, media_type))
            offset += length
