...
test("loads SMTP settings, fixed sender, and reply-to without retaining the saved password mask", async () => {
  const client = { async request() { return { data: { configured: true, host: "smtp.example.test", port: 587, tls_mode: "starttls", username: "mailer", password_masked: "********", enabled: true, version: 3, sender_name: "星河招聘", sender_address: "jobs@example.test", default_reply_to_email: "hr@example.test", default_reply_to_name: "招聘团队" } }; } };

  const settings = await createEmailSettingsController({ client }).load();

  assert.deepEqual(settings, { configured: true, host: "smtp.example.test", port: 587, tlsMode: "starttls", username: "mailer", enabled: true, version: 3, senderName: "星河招聘", senderAddress: "jobs@example.test", defaultReplyToEmail: "hr@example.test", defaultReplyToName: "招聘团队" });
  assert.equal(Object.hasOwn(settings, "password"), false);
  assert.equal(Object.hasOwn(settings, "passwordMasked"), false);
});

test("saves default reply-to and an explicit password replacement with If-Match without echoing it", async () => {
  const calls = [];
  ...
  const saved = await controller.save({ host: "smtp.example.test", port: 465, tlsMode: "tls", username: "mailer", enabled: true, version: 3, defaultReplyToEmail: "hr@example.test", defaultReplyToName: "招聘团队" }, "new-secret");

  assert.deepEqual(calls, [{ path: "/api/v1/settings/email", options: { method: "PUT", ifMatch: '"3"', idempotencyKey: "email-settings-key", body: { host: "smtp.example.test", port: 465, tls_mode: "tls", username: "mailer", enabled: true, default_reply_to_email: "hr@example.test", default_reply_to_name: "招聘团队", password: "new-secret" } } }]);
