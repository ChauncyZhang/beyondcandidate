...
import { getInterviewPrimaryAction } from "./interviewViewState.js";
...
import { CandidateOfferView } from "./OfferViews.jsx";
...
export function candidateInterviewActions(interview, role, currentUserId, scheduleAvailable, feedbackAvailable) {
  if (!interview?.interviewId) return [];
  const actions = [];
  if (scheduleAvailable && canPerformAction(role, "安排面试") && ["已安排", "已确认"].includes(interview.status)) {
    actions.push({ kind: "reschedule", label: "修改面试" });
  }
  const feedbackAction = feedbackAvailable && canPerformAction(role, "提交面试反馈")
    ? getInterviewPrimaryAction(interview, { canSchedule: false, userId: currentUserId })
    : null;
  if (feedbackAction?.kind === "feedback") actions.push(feedbackAction);
  return actions;
}

export function selectCurrentCandidateInterview(interviews, now = new Date()) {
  const records = Array.isArray(interviews) ? interviews.filter((item) => item?.interviewId) : [];
  const reference = now instanceof Date ? now.getTime() : new Date(now).getTime();
  const upcoming = records
    .filter((item) => ["已安排", "已确认"].includes(item.status) && new Date(item.startsAt).getTime() >= reference)
    .sort((left, right) => new Date(left.startsAt).getTime() - new Date(right.startsAt).getTime());
  if (upcoming.length) return upcoming[0];
  return records.find((item) => item.feedbackStatus === "待反馈")
    || records.find((item) => ["已安排", "已确认"].includes(item.status))
    || null;
}
...
export function candidateDetailTabs(serverBacked) {
  return serverBacked
    ? ["档案与简历", "职位申请", "筛选证据", "面试与反馈", "Offer", "时间线"]
...
function CandidateDetail({ candidate, role, currentUserId, onBack, backLabel, onUpdate, onNotify, onScheduleInterview, onOpenInterviewFeedback, onAddToTalentPool, actorName, controller, offerController, offerApprovalId, interviewLoadState, onRefresh, activeTab = "档案与简历", onTabChange = () => {} }) {
  ...
  const canManageOffer = candidate.serverBacked && candidate.stage === "已通过" && canPerformAction(role, "管理 Offer");
  const canOpenGovernance = candidate.serverBacked && canReadCandidateGovernance(role);
  const interviewLoadFailed = candidate.serverBacked && interviewLoadState?.interviewsStatus === "error";
  const currentInterview = candidate.stage === "面试中" ? selectCurrentCandidateInterview(candidate.interviews) : null;
  const currentInterviewActions = interviewLoadFailed ? [] : candidateInterviewActions(currentInterview, role, currentUserId, onScheduleInterview, onOpenInterviewFeedback);
  ...
  return <div className="candidate-page candidate-detail-page">
    ...
    <div className="candidate-detail-layout"><main className="candidate-detail-main"><section className="candidate-detail-panel"><div className="candidate-detail-tabs">{tabs.map((item) => <button type="button" key={item} className={tab === item ? "active" : ""} onClick={() => onTabChange(item)}>{item}</button>)}</div>
      ...
      {tab === "面试与反馈" && <div className="candidate-tab-content"><div className="candidate-interview-toolbar"><div><h3>面试记录</h3><span>可在当前记录中直接改期或填写评价，状态会随操作自动更新。</span></div>{canScheduleCurrent && <button className="button primary" type="button" onClick={() => onScheduleInterview(candidate)}><CalendarDays size={16} />安排面试</button>}</div>{interviewLoadFailed ? <div className="candidate-empty compact error"><CircleAlert size={23} /><strong>面试记录加载失败</strong><span>当前不能进行改期或评价，请重新加载后再试。</span><button className="button primary" type="button" onClick={() => void onRefresh()}><RotateCcw size={16} />重新加载</button></div> : candidate.interviews.length ? <div className="interview-feedback-list">{candidate.interviews.map((item) => { const actions = candidateInterviewActions(item, role, currentUserId, onScheduleInterview, onOpenInterviewFeedback); return <section key={item.interviewId || item.time}><header><div><strong>{item.round}</strong><span>{item.time}</span></div><span className="feedback-result">{item.result}</span></header><p>面试官：{item.interviewer}</p><blockquote>{item.feedback}</blockquote><div className="candidate-interview-actions">{actions.map((action) => <button className={`button ${action.kind === "feedback" ? "primary" : "secondary"}`} type="button" key={action.kind} onClick={() => action.kind === "reschedule" ? onScheduleInterview(candidate, item.interviewId) : onOpenInterviewFeedback(item.interviewId)}>{action.kind === "reschedule" ? <CalendarDays size={15} /> : <MessageSquareText size={15} />}{action.label}</button>)}{actions.length === 0 && onOpenInterviewFeedback && item.interviewId && <button className="button secondary" type="button" onClick={() => onOpenInterviewFeedback(item.interviewId)}>查看材料与评价</button>}</div></section>; })}</div> : <div className="candidate-empty compact"><MessageSquareText size={23} /><strong>暂无面试记录</strong><span>{candidate.stage === "待安排" ? "可以为该候选人创建第一场面试。" : "面试将在 HR 完成安排后显示。"}</span>{canScheduleCurrent && <button className="button primary" type="button" onClick={() => onScheduleInterview(candidate)}><CalendarDays size={16} />安排面试</button>}</div>}</div>}
...
export function CandidatesWorkspace({ mode, setMode, selectedCandidate, setSelectedCandidate, records, setRecords, onNotify, onBackDetail, detailBackLabel, onOpenCandidate, onScheduleInterview, onOpenInterviewFeedback, onAddToTalentPool, filters, onFiltersChange, detailTab, onDetailTabChange, actorName = "张小北", currentUserId, currentRole, controller, offerController, offerApprovalId, detailState, onRetryDetail, pageActionHost, onImport }) {
  ...
  if (mode === "detail" && selectedCandidate) return <CandidateDetail candidate={selectedCandidate} role={currentRole} currentUserId={currentUserId} onBack={onBackDetail || (() => { setSelectedCandidate(null); setMode("list"); })} backLabel={detailBackLabel} onUpdate={updateCandidate} onNotify={onNotify} onScheduleInterview={onScheduleInterview} onOpenInterviewFeedback={onOpenInterviewFeedback} onAddToTalentPool={onAddToTalentPool} actorName={actorName} controller={controller} offerController={offerController} offerApprovalId={offerApprovalId} interviewLoadState={detailState} onRefresh={onRetryDetail} activeTab={detailTab} onTabChange={onDetailTabChange} />;
