...
function OrganizationSettings({
  ...
  const [scopeForm, setScopeForm] = useState({ role: "", recruitingScopeType: "jobs", recruitingDepartmentIds: [] });
  ...
  function openRecruitingScope(user) {
    controller.dismissInvitation();
    setScopeMember(user);
    setScopeForm({ role: user.roleValues[0] || "", recruitingScopeType: user.recruitingScopeType, recruitingDepartmentIds: [...user.recruitingDepartmentIds] });
...
function EmailSettings({ onNotify, onDirtyChange, controller = emailSettingsController }) {
  const empty = { configured: false, host: "", port: 587, tlsMode: "starttls", username: "", enabled: false, version: 0, senderName: "", senderAddress: "", senderSource: "", defaultReplyToEmail: "", defaultReplyToName: "" };
  ...
  return <section className="settings-section email-settings" aria-labelledby="email-settings-title">
    <div className="settings-section-heading"><div><h2 id="email-settings-title"><Mail size={21} />邮件发送设置</h2><p>仅系统管理员可配置组织发件身份和 SMTP 连接；密码只在明确替换时提交。</p></div>{["error", "conflict"].includes(status) && <button className="button secondary" type="button" onClick={() => void load()}><RefreshCw size={16} />重新加载邮件设置</button>}</div>
    {status === "loading" && <div className="organization-state" role="status"><RefreshCw size={18} />正在加载邮件设置…</div>}
    {error && <div className="settings-error" role="alert"><AlertTriangle size={17} />{error}</div>}
    {status !== "loading" && <><form className="email-settings-form" onSubmit={save}><div className="email-form-section"><strong>发件身份</strong><small>将显示在候选人收到的邮件中。发件地址必须已获得当前 SMTP 服务授权；修改仅影响后续邮件。</small></div><label>发件人名称<input required maxLength="200" disabled={busy} value={draft.senderName} onChange={(event) => setDraft({ ...draft, senderName: event.target.value })} placeholder="例如 Aurora 招聘团队" /></label><label>发件地址<input required type="email" maxLength="320" disabled={busy} value={draft.senderAddress} onChange={(event) => setDraft({ ...draft, senderAddress: event.target.value })} placeholder="例如 careers@example.com" /></label><div className="email-form-section"><strong>SMTP 连接</strong><small>用于实际投递邮件。修改连接端点时需要重新输入 SMTP 密码。</small></div><label>SMTP 主机<input required maxLength="253" disabled={busy} value={draft.host} onChange={(event) => setDraft({ ...draft, host: event.target.value })} placeholder="smtp.example.com" /></label><label>端口<input required type="number" min="1" max="65535" disabled={busy} value={draft.port} onChange={(event) => setDraft({ ...draft, port: Number(event.target.value) })} /></label><label>连接加密<select disabled={busy} value={draft.tlsMode} onChange={(event) => setDraft({ ...draft, tlsMode: event.target.value })}><option value="starttls">STARTTLS（连接后升级加密）</option><option value="tls">SSL/TLS（连接即加密）</option></select><small>邮箱服务商标注“SSL”时，请选择“SSL/TLS（连接即加密）”。</small></label><label>用户名<input required maxLength="320" autoComplete="username" disabled={busy} value={draft.username} onChange={(event) => setDraft({ ...draft, username: event.target.value })} /></label><label>默认回复地址<input required type="email" maxLength="320" disabled={busy} value={draft.defaultReplyToEmail} onChange={(event) => setDraft({ ...draft, defaultReplyToEmail: event.target.value })} /></label><label>默认回复名称<input required maxLength="200" disabled={busy} value={draft.defaultReplyToName} onChange={(event) => setDraft({ ...draft, defaultReplyToName: event.target.value })} /></label><label className="email-password-field">替换 SMTP 密码<input type="password" autoComplete="new-password" disabled={busy} value={passwordReplacement} onChange={(event) => setPasswordReplacement(event.target.value)} placeholder={saved.configured ? "留空表示不替换" : "首次配置必须填写"} /><small>系统不会读取、返回或保留已保存密码；首次配置或修改连接端点时必须明确输入替换值。</small></label><label className="email-enabled"><input type="checkbox" disabled={busy} checked={draft.enabled} onChange={(event) => setDraft({ ...draft, enabled: event.target.checked })} />启用此 SMTP 配置</label><div className="email-settings-actions"><span aria-live="polite">{passwordRequired && dirty && !passwordReplacement ? "当前修改需要输入 SMTP 密码替换值。" : dirty ? "有未保存修改；保存前不能发送测试邮件。" : saved.configured ? `已保存版本 ${saved.version}` : "尚未保存 SMTP 配置"}</span><button className="button primary" type="submit" disabled={busy || !dirty || !draft.senderName.trim() || !draft.senderAddress.trim() || !draft.host.trim() || !draft.username.trim() || !draft.defaultReplyToEmail.trim() || !draft.defaultReplyToName.trim() || (passwordRequired && !passwordReplacement)}>{status === "saving" ? "保存中…" : "保存邮件设置"}</button></div></form>
      <section className="email-test-panel"><header><h3>测试已保存配置</h3><p>测试邮件将由 {saved.senderName || "未配置"} {saved.senderAddress || ""} 发出，回复至 {saved.defaultReplyToName || "未配置"} {saved.defaultReplyToEmail || ""}。</p></header><div className="email-test-fields"><label>测试收件人<input type="email" disabled={busy} value={testRecipient} onChange={(event) => setTestRecipient(event.target.value)} /></label><button className="button secondary" type="button" disabled={busy || !testReady} onClick={() => void testSaved()}>{status === "testing" ? "提交中…" : "发送测试邮件"}</button></div></section></>}
  </section>;
}
