...
export function createPublicOfferController({ fetchImpl = globalThis.fetch } = {}) {
  ...
  function normalize(payload) {
    ...
    return { status: text(value.status ?? value.display_status) || "invalid", candidateName: text(value.candidate_name ?? content.candidate_name ?? value.greeting ?? content.greeting), jobTitle: text(value.job_title ?? value.role_title ?? content.job_title), companyName: text(value.company_name ?? value.organization_name ?? content.company_name) || "BeyondCandidate", deadline: text(value.candidate_response_deadline ?? value.deadline ?? content.deadline), summary: text(value.offer_summary ?? content.summary ?? content.body), location: text(value.location ?? value.job_location ?? content.location), contact: text(value.hr_contact ?? value.contact ?? content.hr_contact), pdfAvailable: value.pdf_available === true, response: value.response && typeof value.response === "object" ? { status: text(value.response.status) } : null };
  }
