...
export function ScheduleWorkspace({ record, candidateId, candidates, participantOptions, participantStatus, onRetryParticipants, onBack, backLabel, onSave, onGetAvailability, onNotify }) {
  ...
  return <div className="interview-page schedule-page"><button className="back-link" type="button" onClick={onBack}><ArrowLeft size={17} />{backLabel}</button>
    ...
    <div className="schedule-layout"><main className="schedule-main">
      ...
      {step === 2 && <section className="schedule-section"><header><Users size={19} /><div><h3>选择面试官</h3><p>下一步会直接在日期时间表中标出所选面试官的已占用时段。</p></div></header><div className="interviewer-picker"><strong>面试官</strong><div>{participantOptions.map((person) => <label key={person.id} className={form.interviewerIds.includes(person.id) ? "selected" : ""}><input type="checkbox" checked={form.interviewerIds.includes(person.id)} onChange={() => toggleInterviewer(person.id)} /><span><UserRound size={16} /></span><strong>{person.name}</strong><small>面试参与人</small></label>)}</div>{participantStatus === "loading" && participantOptions.length === 0 && <p className="participant-directory-status"><RefreshCw size={14} />正在加载可选面试官</p>}{participantStatus === "error" && <div className="participant-directory-status error" role="alert"><span><CircleAlert size={14} />面试官目录加载失败或超时，请重新加载</span><button className="button secondary" type="button" onClick={onRetryParticipants}><RefreshCw size={14} />重新加载</button></div>}{errors.interviewers && <p className="field-error">{errors.interviewers}</p>}</div><footer><button className="button secondary" type="button" onClick={() => setStep(1)}>上一步</button><button className="button primary" type="button" disabled={!form.interviewerIds.length} onClick={continueToTime}>下一步：选择日期时间<ChevronRight size={16} /></button></footer></section>}
