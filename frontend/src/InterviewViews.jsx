      ...
      {view === "list" ? <div className="interview-table">
        ...
        {filtered.map((record) => {
          ...
          return <div className="interview-table-row" key={record.id}>
            ...
            <InterviewDeliveryState record={record} />
            <span className="interview-row-actions">
              ...
              {emailActions.resend && <button type="button" disabled={emailAction?.status === "loading"} onClick={() => void onResendEmail(record)}><RefreshCw size={13} />{emailAction?.status === "loading" ? "正在重新发送" : "重新发送"}</button>}
              {emailAction?.error && <small className="interview-action-error" role="alert">{emailAction.error}</small>}
...
export function InterviewsWorkspace({ mode, setMode, selectedInterviewId, setSelectedInterviewId, scheduleCandidateId, records, status, error, onRetry, nextCursor, loadingMore, onLoadMore, candidates, onNotify, onBack, backLabel = "返回面试列表", onOpenSubView, onRecordsChanged, canSchedule = true, canCorrectCandidateEmail = false, candidateEmailController, actorName = "张小北", actorId, controller, pageActionHost }) {
  ...
  async function resendCandidateEmail(record) {
    setResendState((current) => ({ ...current, [record.id]: { status: "loading", error: "" } }));
    try {
      const emailDelivery = await controller.resendEmail(record);
      await onRecordsChanged({ ...record, emailDelivery });
      ...
      if (requiresCandidateEmailCorrection(error) && canCorrectCandidateEmail) {
        ...
        setResendState((current) => ({ ...current, [record.id]: { status: "ready", error: "重新发送失败，请稍后重试。" } }));
      }
    }
  }
