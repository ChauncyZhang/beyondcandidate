...
function CreatePoolDialog({ onClose, onCreate }) {
  ...
  async function submit() { const next = {}; const suitableRoles = parseSuitableRoles(form.suitableRole); if (!form.name.trim()) next.name = "请输入人才库名称"; if (!form.purpose.trim()) next.purpose = "请说明该人才库的用途"; if (suitableRoles.length === 0) next.suitableRole = "请填写至少一个有效的适合岗位"; setErrors(next); if (Object.keys(next).length) return; setSubmitting(true); try { await onCreate({ name: form.name.trim(), purpose: form.purpose.trim(), suitableRoles, owner: form.owner, visibility: form.visibility, retentionDays: Number(form.retentionDays) }); } finally { setSubmitting(false); } }
  return <div className="talent-dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="talent-dialog" role="dialog" aria-modal="true" aria-label="新建人才库" onMouseDown={(event) => event.stopPropagation()}><header><div><h3>新建人才库</h3><p>用于明确沉淀范围、协作权限和保留期限。</p></div><button className="icon-button" type="button" aria-label="关闭" disabled={submitting} onClick={onClose}><X size={19} /></button></header><div className="talent-dialog-body"><label>人才库名称<input value={form.name} disabled={submitting} onChange={(event) => { setForm({ ...form, name: event.target.value }); setErrors({ ...errors, name: "" }); }} placeholder="例如：AI 技术人才" />{errors.name && <small className="field-error">{errors.name}</small>}</label><label>用途说明<textarea rows="3" value={form.purpose} disabled={submitting} onChange={(event) => { setForm({ ...form, purpose: event.target.value }); setErrors({ ...errors, purpose: "" }); }} placeholder="说明什么人才应该进入该分组" />{errors.purpose && <small className="field-error">{errors.purpose}</small>}</label><label>适合岗位<input value={form.suitableRole} disabled={submitting} onChange={(event) => { setForm({ ...form, suitableRole: event.target.value }); setErrors({ ...errors, suitableRole: "" }); }} placeholder="使用顿号、逗号或分号分隔" />{errors.suitableRole && <small className="field-error">{errors.suitableRole}</small>}</label><div className="talent-form-grid"><label>可见范围<select value={form.visibility} disabled={submitting} onChange={(event) => setForm({ ...form, visibility: event.target.value })}><option>招聘团队可见</option><option>仅自己可见</option></select></label><label>默认保留期限<select value={form.retentionDays} disabled={submitting} onChange={(event) => setForm({ ...form, retentionDays: event.target.value })}><option value="365">365 天</option><option value="540">540 天</option><option value="730">730 天</option></select></label></div></div><footer><button className="button secondary" type="button" disabled={submitting} onClick={onClose}>取消</button><button className="button primary" type="button" disabled={submitting} onClick={() => void submit()}>{submitting ? "正在创建" : "创建人才库"}</button></footer></section></div>;
}

function EditPoolDialog({ pool, onClose, onSave }) {
  const [form, setForm] = useState({ name: pool.name, purpose: pool.purpose, suitableRole: pool.suitableRoles.join("；"), visibility: pool.visibility, retentionDays: pool.retentionDays });
  const [errors, setErrors] = useState({});
  const [submitting, setSubmitting] = useState(false);
  async function submit() {
    const suitableRoles = parseSuitableRoles(form.suitableRole);
    const next = {};
    if (!form.name.trim()) next.name = "请输入人才库名称";
    if (!form.purpose.trim()) next.purpose = "请填写用途或说明";
    const retentionDays = Number(form.retentionDays);
    if (!Number.isInteger(retentionDays) || retentionDays < 30 || retentionDays > 3650) next.retentionDays = "请输入 30 至 3650 天";
    setErrors(next);
    if (Object.keys(next).length) return;
    setSubmitting(true);
    try {
      const saved = await onSave({ name: form.name.trim(), purpose: form.purpose.trim(), suitableRoles, visibility: form.visibility, retentionDays });
      if (saved) onClose();
    } finally {
      setSubmitting(false);
    }
  }
  return <div className="talent-dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="talent-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-pool-title" onMouseDown={(event) => event.stopPropagation()}><header><div><h3 id="edit-pool-title">编辑人才库</h3><p>更新人才库用途、适用岗位和数据保留规则。</p></div><button className="icon-button" type="button" aria-label="关闭" disabled={submitting} onClick={onClose}><X size={19} /></button></header><div className="talent-dialog-body"><label>人才库名称<input value={form.name} disabled={submitting} onChange={(event) => setForm({ ...form, name: event.target.value })} />{errors.name && <small className="field-error" role="alert">{errors.name}</small>}</label><label>用途/说明<textarea rows="3" value={form.purpose} disabled={submitting} onChange={(event) => setForm({ ...form, purpose: event.target.value })} />{errors.purpose && <small className="field-error" role="alert">{errors.purpose}</small>}</label><label>适合岗位<input value={form.suitableRole} disabled={submitting} onChange={(event) => setForm({ ...form, suitableRole: event.target.value })} placeholder="使用顿号、逗号或分号分隔" />{errors.suitableRole && <small className="field-error" role="alert">{errors.suitableRole}</small>}</label><div className="talent-form-grid"><label>可见范围<select value={form.visibility} disabled={submitting} onChange={(event) => setForm({ ...form, visibility: event.target.value })}><option>招聘团队可见</option><option>仅自己可见</option><option>指定成员可见</option></select></label><label>保留天数<input type="number" min="30" max="3650" value={form.retentionDays} disabled={submitting} onChange={(event) => setForm({ ...form, retentionDays: event.target.value })} />{errors.retentionDays && <small className="field-error" role="alert">{errors.retentionDays}</small>}</label></div></div><footer><button className="button secondary" type="button" disabled={submitting} onClick={onClose}>取消</button><button className="button primary" type="button" disabled={submitting} onClick={() => void submit()}>{submitting ? "正在保存" : "保存修改"}</button></footer></section></div>;
}

function MoveMembershipDialog({ member, candidate, pool, pools, onClose, onMove }) {
  const [targetPoolId, setTargetPoolId] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState("");
  const [mismatchConfirmed, setMismatchConfirmed] = useState(false);
  const availablePools = pools.filter((item) => item.id !== pool.id && !item.systemKey);
  const targetPool = availablePools.find((item) => item.id === targetPoolId);
  const roleMismatch = Boolean(targetPool && isTalentPoolRoleMismatch(targetPool, member.suitableRoles));

  async function submit() {
    if (!targetPoolId || submitting || (roleMismatch && !mismatchConfirmed)) return;
    setSubmitting(true);
    setError("");
    try {
      await onMove(member, targetPoolId);
      onClose();
    } catch {
      setError("移动失败，请刷新后重试");
    } finally {
      setSubmitting(false);
    }
  }

  return <div className="talent-dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="talent-dialog move-membership-dialog" role="dialog" aria-modal="true" aria-labelledby="move-membership-title" onMouseDown={(event) => event.stopPropagation()}><header><div><h3 id="move-membership-title">移动人才库</h3><p>调整 {candidate.name} 的人才库归类。</p></div><button className="icon-button" type="button" aria-label="关闭" disabled={submitting} onClick={onClose}><X size={19} /></button></header><div className="talent-dialog-body"><dl className="move-membership-summary"><div><dt>当前人才库</dt><dd>{pool.name}</dd></div><div><dt>候选人</dt><dd>{candidate.name}</dd></div></dl><label>目标人才库<select aria-label="选择目标人才库" value={targetPoolId} disabled={submitting} onChange={(event) => { setTargetPoolId(event.target.value); setMismatchConfirmed(false); }}><option value="">请选择目标人才库</option>{availablePools.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>{roleMismatch && <label className="talent-pool-mismatch"><span><CircleAlert size={17} /><strong>岗位与人才库不一致</strong></span><small>该候选人的适合岗位不在“{targetPool.name}”的人才范围中。</small><span><input type="checkbox" checked={mismatchConfirmed} disabled={submitting} onChange={(event) => setMismatchConfirmed(event.target.checked)} />我确认仍要移动到该人才库</span></label>}<p className="field-hint">移动后会保留候选人档案、历史申请和跟进记录，只调整所在人才库。</p>{availablePools.length === 0 && <p className="field-error" role="alert">暂无其他可用人才库，请先新建人才库。</p>}{error && <p className="field-error" role="alert">{error}</p>}</div><footer><button className="button secondary" type="button" disabled={submitting} onClick={onClose}>取消</button><button className="button primary" type="button" disabled={!targetPoolId || submitting || (roleMismatch && !mismatchConfirmed)} onClick={() => void submit()}>{submitting ? "正在移动" : "确认移动"}</button></footer></section></div>;
}
...
function MemberDrawer({ member, candidate, pool, pools, onClose, onUpdate, onMove, onRemove, onRefer, referring, positions, onReactivateCandidate, onOpenCandidate, onNotify, initialReactivateOpen = false }) {
  ...
  async function confirmMove() {
    if (!moveTargetPoolId || pool.systemKey) return;
    setMoveSubmitting(true);
    setMoveError("");
    try {
      await onMove(member, moveTargetPoolId);
    } catch (error) {
      setMoveError(error?.code === "talent_pool_membership_exists" ? "该候选人已经在目标人才库中" : "移动失败，请刷新后重试");
    } finally {
...
function ReviewReferralDialog({ member, onClose, onLoadOptions, onConfirm, submitting }) {
  ...
  useEffect(() => {
    ...
    setStatus("loading");
    setError("");
    onLoadOptions(member.originalJob.id, { signal: request.signal }).then((records) => {
      setOptions(records);
      setAssigneeId(records.length === 1 ? records[0].id : "");
      setStatus("ready");
    }).catch((loadError) => {
      if (loadError?.name !== "AbortError") {
        setStatus("error");
        setError("可选负责人加载失败，请重试。");
      }
    });
    return () => request.abort();
  }, [member.originalJob.id, onLoadOptions]);

  return <div className="talent-dialog-backdrop" role="presentation" onMouseDown={() => !submitting && onClose()}><section className="talent-dialog review-referral-dialog" role="dialog" aria-modal="true" aria-labelledby="review-referral-title" onKeyDown={(event) => { if (event.key === "Escape" && !submitting) onClose(); }} onMouseDown={(event) => event.stopPropagation()}><header><div><h3 id="review-referral-title">选择评审接收人</h3><p>确认后，候选人将回到原岗位的用人经理评审流程。</p></div><button className="icon-button" type="button" aria-label="关闭转交对话框" disabled={submitting} onClick={onClose}><X size={19} /></button></header><div className="talent-dialog-body"><dl className="review-referral-context"><div><dt>候选人</dt><dd>{member.candidate.name}</dd></div><div><dt>原岗位</dt><dd>{member.originalJob.title || "来源申请不可见"}</dd></div></dl>{status === "loading" ? <div className="review-referral-state" role="status"><RefreshCw size={17} />正在加载可选负责人…</div> : status === "error" ? <div className="review-referral-state error" role="alert"><CircleAlert size={17} /><span>{error}</span><button type="button" onClick={() => { setStatus("loading"); setError(""); onLoadOptions(member.originalJob.id).then((records) => { setOptions(records); setAssigneeId(records.length === 1 ? records[0].id : ""); setStatus("ready"); }).catch(() => { setStatus("error"); setError("可选负责人加载失败，请重试。"); }); }}>重试</button></div> : options.length === 0 ? <div className="review-referral-state error" role="alert"><CircleAlert size={17} />原岗位暂无可选负责人，请先编辑职位配置。</div> : <label htmlFor="review-referral-assignee">接收人<span className="required-label">必选</span><select id="review-referral-assignee" autoFocus value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)}><option value="">请选择接收人</option>{options.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}</div><footer><button className="button secondary" type="button" disabled={submitting} onClick={onClose}>取消</button><button className="button primary" type="button" disabled={submitting || status !== "ready" || !assigneeId} onClick={() => void onConfirm(assigneeId)}>{submitting ? "正在转交…" : "确认转交"}</button></footer></section></div>;
}

function DeferredPoolDetail({ pool, pools, memberships, candidates, positions, onBack, onUpdateMember, onMove, onRemove, onReferMember, onLoadReviewerOptions, onReactivateCandidate, onOpenCandidate, onNotify, status, error, onRetry, nextCursor, loadingMore, onLoadMore }) {
  const [query, setQuery] = useState("");
  ...
  function openReferral(member) {
    if (member.sourceStage === "用人经理复核" || referringMemberId) return;
    setReferralTarget(member);
  }

  async function refer(member, assigneeId) {
    if (member.sourceStage === "用人经理复核" || referringMemberId) return;
    setReferringMemberId(member.id);
    setReferralMessage(`正在将 ${member.candidate.name} 转交所选负责人`);
    try {
      await onReferMember(member, assigneeId);
      setReferralMessage(`${member.candidate.name} 已转交所选负责人`);
  ...
  return <div className="talent-page pool-detail-page">
    ...
    <section className="pool-detail-panel">
      ...
      {status === "loading" && memberships.length === 0 ? <div className="talent-empty" role="status"><RefreshCw size={24} /><strong>正在加载未进入评审人才</strong></div> : status === "error" && memberships.length === 0 ? <div className="talent-empty" role="alert"><CircleAlert size={24} /><strong>未进入评审人才加载失败</strong><span>{error}</span><button className="button secondary" type="button" onClick={onRetry}>重试</button></div> : <div className="talent-table" role="table" aria-label="AI 初筛未进入评审人才">
        ...
        {filtered.map(({ member, candidate }) => <div className="talent-table-row" role="row" key={member.id}>
          ...
          <span role="cell" data-label="操作"><button className="button primary small" type="button" aria-label={`选择评审接收人：${candidate.name}`} disabled={member.sourceStage === "用人经理复核" || Boolean(referringMemberId)} onClick={() => openReferral(member)}>{member.sourceStage === "用人经理复核" ? "已转交用人经理" : referringMemberId === member.id ? "正在转交" : "转交用人经理"}</button></span>
        </div>)}
    ...
    {selected && <MemberDrawer key={selected.member.id} member={selected.member} candidate={selected.candidate} pool={pool} pools={pools} positions={positions} onClose={() => setSelectedMemberId(null)} onUpdate={onUpdateMember} onMove={onMove} onRemove={(member) => { onRemove(member); setSelectedMemberId(null); }} onRefer={openReferral} referring={Boolean(referringMemberId)} onReactivateCandidate={onReactivateCandidate} onOpenCandidate={onOpenCandidate} onNotify={onNotify} />}
    {referralTarget && <ReviewReferralDialog member={referralTarget} submitting={referringMemberId === referralTarget.id} onClose={() => setReferralTarget(null)} onLoadOptions={onLoadReviewerOptions} onConfirm={(assigneeId) => refer(referralTarget, assigneeId)} />}
  </div>;
}

function PoolDetail({ pool, pools, memberships, candidates, positions, onBack, onUpdatePool, onDeletePool, onUpdateMember, onMove, onRemove, onReferMember, onLoadReviewerOptions, onReactivateCandidate, onOpenCandidate, onNotify, status = "ready", error = "", onRetry, nextCursor, loadingMore, onLoadMore }) {
  ...
  const [editOpen, setEditOpen] = useState(false); const [deleteOpen, setDeleteOpen] = useState(false); const [movingMemberId, setMovingMemberId] = useState(null); const [actionMessage, setActionMessage] = useState(""); const [deleting, setDeleting] = useState(false);
  ...
  const moving = poolMembers.find((item) => item.member.id === movingMemberId);
  const memberTotal = pool.memberCount ?? poolMembers.length;
  if (pool.systemKey === "ai_screening_deferred") return <DeferredPoolDetail pool={pool} pools={pools} memberships={memberships} candidates={candidates} positions={positions} onBack={onBack} onUpdateMember={onUpdateMember} onMove={onMove} onRemove={onRemove} onReferMember={onReferMember} onLoadReviewerOptions={onLoadReviewerOptions} onReactivateCandidate={onReactivateCandidate} onOpenCandidate={onOpenCandidate} onNotify={onNotify} status={status} error={error} onRetry={onRetry} nextCursor={nextCursor} loadingMore={loadingMore} onLoadMore={onLoadMore} />;
  return <div className="talent-page pool-detail-page"><button className="back-link" type="button" onClick={onBack}><ArrowLeft size={17} />返回人才库</button><section className="pool-detail-hero"><div><span><BriefcaseBusiness size={20} /></span><div><div><h2>{pool.name}</h2><VisibilityTag value={pool.visibility} /></div><p>{pool.purpose}</p><small>适合岗位：{pool.suitableRoles.join("、") || "未设置"} · 人才库负责人：{pool.owner} · 默认保留 {pool.retentionDays} 天 · 最近活动 {pool.recentActivity}</small></div></div><div className="pool-detail-summary"><strong>{memberTotal}<small>人才</small></strong><div className="pool-detail-actions"><button className="button secondary small" type="button" onClick={() => setEditOpen(true)}><Pencil size={15} />编辑</button><button className="button secondary small" type="button" onClick={() => { if (memberTotal > 0) setActionMessage("该人才库仍有候选人，请先移动或移除后再删除。"); else setDeleteOpen(true); }}><Trash2 size={15} />删除</button></div></div></section>{actionMessage && <p className="pool-action-message" role="alert">{actionMessage}</p>}<section className="pool-detail-panel"><div className="pool-detail-toolbar"><label className="pool-search"><Search size={16} /><input aria-label="搜索人才" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索姓名、技能或标签" /></label><label className="pool-select"><select aria-label="适合岗位筛选" value={role} onChange={(event) => setRole(event.target.value)}><option>全部适合岗位</option>{[...new Set(poolMembers.flatMap((item) => item.member.suitableRoles))].map((item) => <option key={item}>{item}</option>)}</select><ChevronDown size={14} /></label><label className="pool-select"><select aria-label="城市筛选" value={city} onChange={(event) => setCity(event.target.value)}><option>全部城市</option>{[...new Set(poolMembers.map((item) => item.candidate.city))].map((item) => <option key={item}>{item}</option>)}</select><ChevronDown size={14} /></label><label className="pool-select"><select aria-label="跟进人筛选" value={owner} onChange={(event) => setOwner(event.target.value)}><option>全部跟进人</option>{[...new Set(poolMembers.map((item) => item.member.owner))].map((item) => <option key={item}>{item}</option>)}</select><ChevronDown size={14} /></label><label className="pool-select"><select aria-label="跟进状态筛选" value={followup} onChange={(event) => setFollowup(event.target.value)}><option>全部跟进</option><option>7 天内联系</option><option>即将到期</option></select><ChevronDown size={14} /></label></div>{status === "loading" && memberships.length === 0 ? <div className="talent-empty" role="status"><RefreshCw size={24} /><strong>正在加载人才</strong></div> : status === "error" && memberships.length === 0 ? <div className="talent-empty" role="alert"><CircleAlert size={24} /><strong>人才列表加载失败</strong><span>{error}</span><button className="button secondary" type="button" onClick={onRetry}>重试</button></div> : <div className="talent-table"><div className="talent-table-head"><span>人才</span><span>适合岗位</span><span>技能与标签</span><span>历史职位/结论</span><span>跟进人</span><span>下次联系</span><span>保留期限</span><span>操作</span></div>{filtered.map(({ member, candidate }) => <div className="talent-table-row" key={member.id}><button className="talent-person" type="button" onClick={() => setSelectedMemberId(member.id)}><span>{candidate.name.slice(-1)}</span><span><strong>{candidate.name}</strong><small>{candidate.role} · {candidate.city}</small></span></button><span>{member.suitableRoles.join("、") || "未设置"}</span><span><div className="talent-row-tags">{[...candidate.skills.slice(0, 2), ...member.tags.slice(0, 1)].map((item) => <small key={item}>{item}</small>)}</div></span><span><strong>{candidate.applications.map((item) => item.position).join("、")}</strong><small>{member.latestConclusion}</small></span><span>{member.owner}</span><span><strong>{member.nextContact || "未设置"}</strong><small>{member.recentInteraction}</small></span><span className={member.status === "即将到期" ? "retention-warning" : ""}>{member.retentionUntil}<small>{member.status}</small></span><span className="talent-row-actions"><button className="button secondary small" type="button" onClick={() => setMovingMemberId(member.id)}><ArrowRightLeft size={14} />移动人才库</button><button className="button primary small" type="button" onClick={() => { setSelectedMemberId(member.id); setReactivateMemberId(member.id); }}>重新激活</button></span></div>)}{filtered.length === 0 && <div className="talent-empty"><Search size={24} /><strong>没有符合条件的人才</strong><span>调整筛选条件后重试。</span></div>}{nextCursor && <div className="talent-load-more"><button className="button secondary" type="button" disabled={loadingMore} onClick={onLoadMore}>{loadingMore ? "正在加载" : "加载更多人才"}</button></div>}</div>}</section>{selected && <MemberDrawer key={selected.member.id} member={selected.member} candidate={selected.candidate} pool={pool} pools={pools} positions={positions} initialReactivateOpen={selected.member.id === reactivateMemberId} onClose={() => { setSelectedMemberId(null); setReactivateMemberId(null); }} onUpdate={onUpdateMember} onMove={onMove} onRemove={(member) => { onRemove(member); setSelectedMemberId(null); setReactivateMemberId(null); }} onReactivateCandidate={onReactivateCandidate} onOpenCandidate={onOpenCandidate} onNotify={onNotify} />}{moving && <MoveMembershipDialog member={moving.member} candidate={moving.candidate} pool={pool} pools={pools} onClose={() => setMovingMemberId(null)} onMove={onMove} />}{editOpen && <EditPoolDialog pool={pool} onClose={() => setEditOpen(false)} onSave={onUpdatePool} />}{deleteOpen && <div className="talent-dialog-backdrop" role="presentation" onMouseDown={() => setDeleteOpen(false)}><section className="talent-dialog danger-confirm" role="alertdialog" aria-modal="true" aria-labelledby="delete-pool-title" onMouseDown={(event) => event.stopPropagation()}><header><div><h3 id="delete-pool-title">删除人才库</h3><p>“{pool.name}”为空。删除后无法恢复。</p></div></header><footer><button className="button secondary" type="button" disabled={deleting} onClick={() => setDeleteOpen(false)}>取消</button><button className="button danger" type="button" disabled={deleting} onClick={async () => { setDeleting(true); try { await onDeletePool(); } finally { setDeleting(false); } }}>{deleting ? "正在删除" : "确认删除"}</button></footer></section></div>}</div>;
}
...
export function TalentPoolWorkspace({ mode, setMode, selectedPoolId, setSelectedPoolId, pools = [], setPools, memberships = [], setMemberships, candidates, positions, onReactivateCandidate, onReferralComplete = () => {}, onOpenCandidate, onNotify, controller, actorId, pageActionHost }) {
  ...
  const loadReviewerOptions = useCallback((jobId, options) => controller.listReviewerOptions(jobId, options), [controller]);
  ...
  async function referToReview(member, assigneeId) { try { const result = await controller.referToReview(member.id, member.version, assigneeId); if (!result.membership) throw new Error("referral membership missing"); setServerState((current) => ({ ...current, memberships: current.memberships.map((item) => item.id === result.membership.id ? result.membership : item) })); onReferralComplete(result.application); onNotify("已转交所选负责人"); return result; } catch (error) { const message = reviewReferralErrorMessage(error); if (error && typeof error === "object") error.userMessage = message; onNotify(message); throw error; } }
